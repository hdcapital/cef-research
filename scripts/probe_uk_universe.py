...
for m in months:
    ...
    rec = {
        ...
        "has_price": int(g["price"].notna().sum()),
        "has_nav": int(g["nav"].notna().sum()),
        "has_both": int((g["price"].notna() & g["nav"].notna()).sum()),
...
excluded = g[~g["eligible"] & g["price"].notna()]
...
out["with_nav_last3m"] = int(recent[recent["nav"].notna()]["security_id"].nunique())
out["with_price_last3m"] = int(recent[recent["price"].notna()]["security_id"].nunique())
